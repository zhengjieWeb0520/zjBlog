...
function Toolbar (props) {
  // 底图数组属性
  const { mapView } = props
  const [mapType, setMapType] = useState('gaode_basemap_satelliteMap')
  const toolbarStyle = {
    position: 'absolute',
    right: '1rem',
    top: '0.2rem',
  ...
  const handleChange = (key) => {
    ...
    // 谷歌地图和高德影像显示标注
    if (key === 'gaode_basemap_satelliteMap') {
  ...
  return (
    <div className="toolbar" style={toolbarStyle}>
      ...
      >
        <Option key="gaode_basemap_digitalMap" value="gaode_basemap_digitalMap">
          电子地图
        </Option>
        <Option key="gaode_basemap_satelliteMap" value="gaode_basemap_satelliteMap">
          影像图
        </Option>
      </Select>
    </div>
    ...
    esriLoader
      ...
      .then(([
        ...
        const mapControl = initBaseLayer(
          'gaode',
          ['https://webrd01.is.autonavi.com/appmaptile', 'https://webst01.is.autonavi.com'],
          {
            satelliteUrl: 'https://webst01.is.autonavi.com/appmaptile?style=6&x={x}&y={y}&z={z}',
            digitalUrl: 'https://webrd01.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=8&x={x}&y={y}&z={z}',
            anooMarkerUrl: 'https://webst01.is.autonavi.com/appmaptile?style=8&x={x}&y={y}&z={z}'
          },
          BaseTileLayer,
          Basemap,
          map,
